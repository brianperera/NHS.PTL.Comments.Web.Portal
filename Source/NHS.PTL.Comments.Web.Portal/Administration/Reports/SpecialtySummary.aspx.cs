        ...
        private void GenerateDataGrid(IList<OpReferral> opReferrals)
        {
            if (null != opReferrals)
            {
                ...
                if (statusCount.Count > 0)
                {
                    ...
                    javaScript.Append("]);");
                    javaScript.Append("var options = {title: '',colors: ['#58D3F7'],vAxis: { format: '0'},hAxis: { title: 'Status', titleTextStyle: { color: 'black'} }}; var chart = new google.visualization.ColumnChart(document.getElementById('chart_div')); chart.draw(data, options);}");
